...
class RAGEngine:
    ...
    def generate_response(self, query: str, retrieved_items: List[Dict[str, Any]], history: List[Dict[str, str]] = None) -> str:
        ...
        context_text = ""
        if retrieved_items:
            context_text = "Here are some relevant tasks you might suggest to the user:\n"
            for idx, item in enumerate(retrieved_items):
                context_text += f"[{idx+1}] Title: {item['title']}\nDescription: {item['description']}\nTags: {item['tags']}\nOrganization: {item['category']}\n\n"
        else:
            context_text = "I couldn't find any specific tasks matching the user's query."

        # Construct prompt
        system_prompt = (
            "You are a helpful AI assistant specialized in recommending volunteering tasks to users. "
            "Use the provided context to suggest the most relevant tasks. Be polite, concise, and explain why "
            "they fit the user's request. Do not make up tasks that are not in the context."
        )
